...
public class StarWarEndpoint {
	...
	@RequestMapping(method=RequestMethod.POST, path="/adicionarPlanetas")
	public ResponseEntity<?> adicionarPlanetas() throws URISyntaxException {
	    ...
	    Gson gson = new Gson();
         ...
         for(int i=0; i<parseLista.size(); i++) {
        	 ...
        	 planet.setName((String)parseLista.get(i).get("name"));
        	 planet.setClimate((String)parseLista.get(i).get("climate"));
        	 planet.setTerrain((String)parseLista.get(i).get("terrain"));
        	 planet.setFilms((Integer)((List) parseLista.get(i).get("films")).size());
	...
	@RequestMapping(method=RequestMethod.GET, path="/listAllBd")
	public ResponseEntity<?> listarPlanetasBD(){
